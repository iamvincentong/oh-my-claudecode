...
describe('purgeStalePluginCacheVersions', () => {
  ...
  it('handles multiple marketplaces and plugins', () => {
    ...
    const active2 = join(cacheDir, 'omc/oh-my-openagent/4.3.0');
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      ...
      plugins: {
        ...
        'oh-my-openagent@omc': [{ installPath: active2 }],
      },
    }));
    ...
    mockedReaddirSync.mockImplementation((p, _opts?) => {
      ...
      if (ps.endsWith('omc')) return [dirent('oh-my-openagent')] as any;
      if (ps.endsWith('oh-my-openagent')) return [dirent('4.3.0')] as any;
      return [] as any;
    });
  ...
  it('does nothing when all cache versions are active', () => {
    const cacheDir = '/mock/.claude/plugins/cache';
    const active = join(cacheDir, 'omc/oh-my-openagent/4.3.0');
    ...
    mockedReadFileSync.mockReturnValue(JSON.stringify({
      version: 2,
      plugins: {
        'oh-my-openagent@omc': [{ installPath: active }],
      },
    }));
